...
class ChessBoard(object):
    ...
    def set_state(self):#深拷贝
        new_board = ChessBoard(self.screen)  # 创建新棋盘对象
        new_board.topleft = copy.deepcopy(self.topleft)  # 复制位置属性
        new_board.capture_count = copy.deepcopy(self.capture_count)  # 复制吃子计数
        
        new_board.chessboard_map=self.get_chessboard_str_map()
        # 深拷贝棋盘棋子状态
        for row, line in enumerate(new_board.chessboard_map):
            for col, chess_name in enumerate(line):
                if chess_name:
                    # 将创建的棋子添加到属性map中
                    new_board.chessboard_map[row][col] = Chess(new_board.screen, chess_name, row, col)
                else:
                    new_board.chessboard_map[row][col] = None
        return new_board
    ...
    def move_chess(self, new_row, new_col,is_print=True):  # ToDo: 新增判断是否吃了对方棋子
        """
        将棋子移动到指定位置
        """
        # 得到要移动的棋子的位置
